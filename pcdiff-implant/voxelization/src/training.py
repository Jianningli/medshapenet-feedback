...
from src.utils import grid_interp, export_pointcloud, export_mesh, \
                      mc_from_psr, scale2onet, GaussianSmoothing, rescale
...
class Trainer(object):
    ...
    def save(self, model, data, epoch, id):
        ...
        if exp_pcl:
            ...
            export_pointcloud(os.path.join(dir_pcl, '{:04d}_{:01d}.ply'.format(epoch, id)),
                              points, normals)
            if exp_gt:
                export_pointcloud(os.path.join(dir_pcl, '{:04d}_{:01d}_oracle.ply'.format(epoch, id)),
