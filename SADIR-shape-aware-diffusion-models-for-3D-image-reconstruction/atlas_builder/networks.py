...
class VxmDense(LoadableModel):
    ...
    def EPDiff_step(self, metric, m0, dt, phiinv, mommask=None):
    ...
    def lagomorph_expmap_shooting(self, metric, m0, T=1.0, num_steps=10, phiinv=None, mommask=None, checkpoints=False):
        """
        ...
        if checkpoints is None or not checkpoints:
            ...
            for i in range(num_steps):
                phiinv, m, v = self.EPDiff_step(metric, m0, dt, phiinv, mommask=mommask)

        return phiinv

    def get_deformed_image(self, src, m0):
        idx = 4
        device = torch.device('cuda')
        imagesize = src.size()[-1]
        num_steps = 10
        alpha=0.5; gamma = 1.0; alpha=3
        fluid_params = [alpha, 0, gamma]
        metric = lm.FluidMetric(fluid_params)
        #src = src.permute(0,1,4,3,2)
        # id = torch.from_numpy(lm.identity((1,3,imagesize,imagesize,imagesize))).to(device)

        phiinv = self.lagomorph_expmap_shooting(metric, m0, num_steps=num_steps)
        Sdef = lm.interp(src, phiinv)


        ###  save results  ###
        ##  !!! if you want to save results, you should transpose the axies back to original order after lagomorph  ##
...
#             # resize to final resolution
        ...
        print(pos_flow.size(), source.size())
        y_source, pos_phi = self.get_deformed_image(source, pos_flow)
        # y_target = self.get_deformed_image(target, neg_flow) if self.bidir else None

        # return non-integrated flow field if training
        if not registration:
            return (y_source, y_target, preint_flow) if self.bidir else (y_source, preint_flow)
        else:
            return y_source, pos_phi, latent_f
