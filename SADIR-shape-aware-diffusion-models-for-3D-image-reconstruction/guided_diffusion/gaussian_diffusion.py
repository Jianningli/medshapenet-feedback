...
class GaussianDiffusion:
        ...
        if progress:
           ...
           for i in indices:
                ...
                with th.no_grad():
                    ...
                    #ToDo check if this is correct, automate this
                    #if img.shape != (1, 3, 32, 32, 32):
                    if img.shape != (1, 3, 64, 64, 64):
    ...
    def ddim_sample_loop_progressive(
        ...
        for i in indices:
                t = th.tensor([i] * shape[0], device=device)
                with th.no_grad():
                #ToDo: Automate this image shape
                 if img.shape!= (1, 3, 32,32,32):
                 #if img.shape != (1, 5, 224, 224):
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            if self.model_var_type in [
                ...
                get_vb_terms_outputs=self._vb_terms_bpd(
                ...
                terms["vb"]=get_vb_terms_outputs["output"]
                pred_xstart= get_vb_terms_outputs["pred_xstart"]
            ...
            huberloss = HuberLoss()
            #bce = BCE()
#             error_tensor= (target - model_output) ** 2
#             error_tensor = torch.div((error_tensor - float(torch.min(error_tensor))), (float(torch.max(error_tensor)) - float(torch.min(error_tensor))))
...
#             dice_loss= 1 - dice
            ...
            # terms["mse"]= mean_flat(error_tensor) + (0 * dice_loss)
            # Add dice loss to mse
            if "vb" in terms:
